...
- **Engineering discipline (constitutional, NON-NEGOTIABLE)**:
  - **Strict TDD** — tests precede production code on EVERY phase, including decimal/insertion phases (X.Y). Yolo-mode does NOT exempt from TDD. Each fix lands with its tests in the SAME atomic commit. RED → GREEN → REFACTOR is the only allowed cadence.
  - **Per-phase coverage floor ≥ 90%** on all new/modified code in that phase across ALL FOUR axes: lines, branches, functions, statements. Applies to every package. A phase MAY NOT close — verifier MUST report `gaps_found` — when any new/modified file is < 90% on any axis. No gap-closure exceptions.
  - **E2E is mandatory, not optional** — every phase that touches a user-visible route, wire surface, or operator-facing artifact MUST ship at least one e2e test that boots the real `docker compose` stack (or a hermetic mock-LiteLLM profile when the upstream is a paid SaaS provider) and asserts the route round-trips end-to-end. E2E tests live in `tests/e2e/` and run via `make e2e-test` (gated on `E2E=1` env). Phase verification MUST execute the e2e suite, not just unit tests, before reporting `passed`.
  - **No mocks of internal logic** — mocks are allowed ONLY at process/network boundaries (HTTP clients to third-party SaaS, OS time, filesystem). Mocking a function the route under test calls is forbidden. If an integration is hard to test, write a real testcontainer/integration test, not a mock.
  - **Real services in tests** — `packages/data` and any DB-touching code MUST run testcontainer integration tests against real Postgres + PgBouncer + Valkey. Skipping testcontainers because Docker is unavailable is allowed locally but MUST fail CI; the orchestrator MUST verify Docker is running before claiming a phase passes.
  - **GitHub Actions** is the only sanctioned CI; workflows in `.github/workflows/` from the first commit of phase 0. CI MUST run unit + integration + contract + e2e on every PR; e2e secrets gate only the live-provider matrix, not the wire-shape matrix.
  - **Maximum test automation** — no human QA; coverage spans unit, integration (real services via testcontainers), e2e (live compose stack), contract (against `BACKEND_SPEC.md`), load (1000 concurrent), security (SAST + deps + container + secrets + license), migration safety, i18n completeness, RLS-isolation property tests.
  - **Verification gate** — the gsd-verifier agent MUST execute `make e2e-test` (or its hermetic equivalent) and parse `pnpm -r test --coverage` output. A phase passes only when (a) every must_have observable truth is verified against the live codebase, (b) coverage ≥ 90/90/90/90 on the diff, (c) e2e suite is green. Anything else is `gaps_found`.
