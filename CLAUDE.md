...
- **Engineering discipline (constitutional, NON-NEGOTIABLE)**:
  ...
  - 16. **RLS posture ledger (data:CR-02 / Phase 57 Track B, D2).** The 4 Better Auth identity tables (`users`, `sessions`, `account`, `verification`) resolve to the default tenant absent a `withTenant()` context — migration `0024` re-installed the rolconfig + GUC-bound `tenant_id` column DEFAULTs the Better Auth `drizzleAdapter` (a module-singleton) depends on for its bare INSERTs. The other 12 application tables remain fail-closed (Phase 32 / migration `0018`). This is **accepted v1 single-installation-single-tenant debt** — exactly one tenant exists, so no live cross-tenant exposure. The durable fix (request-scoped per-request Better Auth adapter, "D3") is a **v2-blocker** tracked in `.planning/deferred-items.md`. The cohort boundary is property-tested at `packages/data/tests/unit/__tests__/rls-posture-boundary.test.ts`; full posture documented in `docs/security.md` §11.1.
