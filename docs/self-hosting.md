...
| `DEFAULT_AGENT_MODEL` | optional | `qwen/qwen3.6-plus` | `apps/api/src/routes/agent/stream.ts` | Override the default model id for `/api/agent/stream` requests that do not pass `model:` in the body. The model id MUST be present in the live `compose/litellm/litellm_config.yaml` — operators substituting an internal model also update LiteLLM's config. |
