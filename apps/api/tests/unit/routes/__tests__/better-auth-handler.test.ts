...
describe("Phase 02.4 G5a — better-auth-handler bridge plugin", () => {
  ...
  // Phase 57 / Track E (api-routes-rest:CR-01) — the pre-fix code fell
  // back to req.headers.host when no env var was set. That fallback is
  // removed; validateIngressBoot() guarantees AUTH_URL is present at
  // runtime. With AUTH_URL set, the constructed URL is env-derived and
  // the Host header is never reflected.
  it("URL reconstruction never reflects the Host header (env-derived origin only)", async () => {
    const capture: CapturedRequest[] = [];
    const prevIngress = process.env.INGRESS_BASE_URL;
    const prevAuth = process.env.AUTH_URL;
    delete process.env.INGRESS_BASE_URL;
    process.env.AUTH_URL = "https://canonical.test.invalid";
    try {
      await app.register(
        buildBetterAuthHandlerRoutes({ auth: makeStubAuth({ capture }) as never }),
      );
      ...
      expect(capture[0]?.url).toBe("https://canonical.test.invalid/api/auth/foo");
    } finally {
  ...
  // Phase 57 / Track E (api-routes-rest:CR-01) — buildRequestUrl no
  // longer has a fallback ladder. The canonical origin is read ONLY from
  // INGRESS_BASE_URL (preferred) or AUTH_URL; validateIngressBoot()
  // guarantees one is set. The malformed-URL catch arms, the
  // AUTH_TRUSTED_ORIGINS_EXTRA allowlist branch, and the
  // x-forwarded-proto + Host reconstruction are all removed.
  ...
  it("INGRESS_BASE_URL wins over AUTH_URL when both are set", async () => {
    const capture: CapturedRequest[] = [];
    const prevIngress = process.env.INGRESS_BASE_URL;
    const prevAuth = process.env.AUTH_URL;
    process.env.INGRESS_BASE_URL = "https://ingress.test.invalid";
    process.env.AUTH_URL = "https://auth.test.invalid";
    try {
      await app.register(
        buildBetterAuthHandlerRoutes({ auth: makeStubAuth({ capture }) as never }),
      ...
      await app.inject({ method: "GET", url: "/api/auth/foo" });
      expect(capture[0]?.url).toBe("https://ingress.test.invalid/api/auth/foo");
    } finally {
  ...
  it("strips a trailing slash from the env-derived origin", async () => {
    const capture: CapturedRequest[] = [];
    const prevIngress = process.env.INGRESS_BASE_URL;
    process.env.INGRESS_BASE_URL = "https://ingress.test.invalid/";
    try {
