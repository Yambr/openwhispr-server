...
import { cookieDomainConfig, findSharedParentDomain } from "../../../src/lib/cookie-domain.js";

describe("findSharedParentDomain", () => {
  it("returns the host itself for identical hosts of >=2 labels", () => {
    expect(findSharedParentDomain("api.example.com", "api.example.com")).toBe("api.example.com");
  });

  it("returns the eTLD+1 for siblings under a shared parent", () => {
    expect(findSharedParentDomain("auth.example.com", "api.example.com")).toBe("example.com");
  });
  ...
  it("returns the deeper shared parent when both share more than 2 labels", () => {
    expect(findSharedParentDomain("a.svc.example.com", "b.svc.example.com")).toBe(
      "svc.example.com",
    );
  });
});
