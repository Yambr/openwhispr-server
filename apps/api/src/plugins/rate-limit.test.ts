...
import { GLOBAL_IP_CEILING, rateLimits, routeRateLimitConfig } from "../config/rate-limits.js";
...
describe("rate-limit IP-tier with injected redis-like store", () => {
  ...
  it("redisIpStore.incr increments and sets TTL on first hit only", async () => {
    ...
    await app.register(rateLimitPlugin, {
      // biome-ignore lint/suspicious/noExplicitAny: opaque test injection
      redis: client as any,
      globalIpCeiling: 2,
...
describe("rate-limit config env override", () => {
  it("num() honours RATE_LIMIT_* env vars over the default", async () => {
    ...
    vi.resetModules();
    const mod = await import("../config/rate-limits.js");
