...
test.describe("U5 — account (Phase 07.1 / Plan 08)", () => {
  ...
  test("empty state — single session hides 'Revoke all other sessions'", async ({ page }) => {
    await page.goto("/app/account");
  ...
  test("success state — two sessions render and 'Revoke all other sessions' is visible", async ({
    ...
    await signInAs(secondPage, fixtureEmail(0));
  ...
  test("axe — WCAG 2.2 AA clean on populated account screen", async ({ page }) => {
    await page.goto("/app/account");
    await expect(page.getByText(fixtureEmail(0))).toBeVisible();
    await runAxe(page);
  });
