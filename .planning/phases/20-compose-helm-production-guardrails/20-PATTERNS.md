...
**Primary analog:** `/Users/dev/openwhispr-server/tools/lint-traefik-routes.ts`
...
**Primary analog:** `/Users/dev/openwhispr-server/tools/lint-traefik-routes.test.ts` (lines 1-45 — read whole file).
...
**Primary analog:** `/Users/dev/openwhispr-server/tools/__tests__/fixtures/traefik-routes/{good,bad}/compose/*.yml`
...
**Location correction:** the spec mentions `tests/openwhispr/*.yaml` but that path does NOT exist. The real path is `/Users/dev/openwhispr-server/charts/openwhispr/tests/` (verified via `find`). 22 existing `*_test.yaml` files live there including `api_test.yaml`, `web_test.yaml`, `worker_test.yaml`, `litellm_test.yaml`, `otel_test.yaml`. **Phase 20 should EXTEND those existing suites with new `tests:` entries rather than create parallel files.**

**Primary analog for assertions:** `/Users/dev/openwhispr-server/charts/openwhispr/tests/api_test.yaml:83-99` (readinessProbe assertions) and `:91-100` (path/port/initialDelay equality).
...
**Primary analog (for `deploy.resources.limits` shape):** `/Users/dev/openwhispr-server/compose/docker-compose.load-test.yml` (already declares limits per docs/CONTEXT) — read for canonical format.
...
**Primary analog:** `/Users/dev/openwhispr-server/Makefile:25-26` (the `lint-rls` target).
```makefile
lint-rls:
...
**Analog search scope:**
- `/Users/dev/openwhispr-server/tools/` (50 entries, focused on lint-* prefix)
- `/Users/dev/openwhispr-server/tools/__tests__/fixtures/` (2 existing fixture trees)
- `/Users/dev/openwhispr-server/charts/openwhispr/templates/` (46 templates)
- `/Users/dev/openwhispr-server/charts/openwhispr/tests/` (22 helm-unittest suites)
- `/Users/dev/openwhispr-server/.github/workflows/` (17 workflows; closest: helm-lint.yml, ci.yml)
- `/Users/dev/openwhispr-server/compose/` (13 compose overlays)
- `/Users/dev/openwhispr-server/apps/{api,web,worker}/Dockerfile`
