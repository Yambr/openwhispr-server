...
- `/Users/dev/openwhispr-server/docker-compose.yml` — base compose, 6 services without `deploy.resources`
- `/Users/dev/openwhispr-server/compose/docker-compose.observability.yml` — 5 LGTM services missing `restart`
- `/Users/dev/openwhispr-server/compose/docker-compose.storage.yml` — minio missing `restart`
- `/Users/dev/openwhispr-server/compose/docker-compose.pgbouncer.yml` — pgbouncer missing `restart`
- `/Users/dev/openwhispr-server/compose/docker-compose.ingress.yml` — traefik missing `restart`
- `/Users/dev/openwhispr-server/apps/api/Dockerfile:172` — `USER node` (uid 1000)
- `/Users/dev/openwhispr-server/apps/worker/Dockerfile:80` — `USER node` (uid 1000)
- `/Users/dev/openwhispr-server/apps/web/Dockerfile:117-118, 132` — `USER nextjs` (uid 1001)
...
- `/Users/dev/openwhispr-server/charts/openwhispr/templates/api-deployment.yaml` — current shape, no securityContext, no startupProbe, no topology
- `/Users/dev/openwhispr-server/charts/openwhispr/templates/web-deployment.yaml` — same gaps
- `/Users/dev/openwhispr-server/charts/openwhispr/templates/worker-deployment.yaml` — same gaps + pgrep exec probe at lines 105-120
- `/Users/dev/openwhispr-server/charts/openwhispr/templates/litellm-deployment.yaml` — same gaps + `/health/liveliness` probe at line 99
- `/Users/dev/openwhispr-server/charts/openwhispr/templates/otel-collector-daemonset.yaml:81-86` — partial hardening; missing `allowPrivilegeEscalation` + `seccompProfile`
- `/Users/dev/openwhispr-server/charts/openwhispr/values.yaml:79-426` — current values surface, no securityContext/topologySpread blocks
- `/Users/dev/openwhispr-server/charts/openwhispr/tests/api_test.yaml` — canonical helm-unittest patterns (assertion syntax verified)
- `/Users/dev/openwhispr-server/tools/lint-traefik-routes.ts` + `.test.ts` — canonical lint tool pattern (Phase 19b)
- `/Users/dev/openwhispr-server/tools/lint-compose-chart-parity.ts:28-110` — COMPOSE_FILES union pattern + CI integration shape
- `/Users/dev/openwhispr-server/.github/workflows/helm-lint.yml` — CI workflow template
- `/Users/dev/openwhispr-server/.planning/phases/20-compose-helm-production-guardrails/20-AUDIT-SOURCE.md` — audit findings A1-A10, B1-B10, C1-C10
- `/Users/dev/openwhispr-server/.planning/phases/20-compose-helm-production-guardrails/20-CONTEXT.md` — locked decisions SR-20.1..SR-20.7
