...
upstream desktop client repo `/Users/dev/openwhispr` is READ-ONLY for us.
...
messages?) and surface the RECOMMENDED option first with rationale.
  ...
  <options>
    ...
    <option id="option-b">
      ...
      <cons>Requires a coordinated upstream change: the client `ConversationsService.ts` interface must follow. The client repo is READ-ONLY — we can only edit `packages/wire-schemas` and must FLAG the required upstream change in the SUMMARY; we MUST NOT touch `/Users/dev/openwhispr`.</cons>
    </option>
  </options>
...
- **If option-b:** `packages/wire-schemas/src/conversations.ts:19` — add
  ...
  `/Users/dev/openwhispr`. The SUMMARY MUST FLAG: the upstream client
...
- H-2: advisor checkpoint resolved; RED+GREEN pair — server role enum and
  ...
  follow-up is FLAGGED in the SUMMARY and `/Users/dev/openwhispr` is untouched.
...
| H-2 option-b: temptation to edit the read-only client repo. | 3 | The plan explicitly forbids touching `/Users/dev/openwhispr`; the upstream change is FLAGGED in the SUMMARY only. |
