...
- `/Users/nick/openwhispr-server/docker-compose.yml` (lines 1-865) — service inventory + Speaches absence in default profile
- `/Users/nick/openwhispr-server/docker-compose.load-test.yml` (lines 340-435) — Speaches block + HF_TOKEN consumption
- `/Users/nick/openwhispr-server/docker-compose.load-test.realistic.yml` — realistic profile overlay
- `/Users/nick/openwhispr-server/charts/openwhispr/values.yaml` (lines 1-345) — chart defaults including `bundledAi.enabled: false`, `litellm.embedded: true`
- `/Users/nick/openwhispr-server/charts/openwhispr/templates/secrets.yaml` — 15-key required list
- `/Users/nick/openwhispr-server/charts/openwhispr/templates/externalsecret.yaml` — ESO data block
- `/Users/nick/openwhispr-server/charts/openwhispr/templates/litellm-deployment.yaml` — embedded=false short-circuit
- `/Users/nick/openwhispr-server/charts/openwhispr/templates/{api,web,worker}-deployment.yaml` — initContainer env enumeration
- `/Users/nick/openwhispr-server/charts/openwhispr/examples/values-corporate-litellm.yaml` — existing Variant B skeleton
- `/Users/nick/openwhispr-server/charts/openwhispr/examples/values-oss-quickstart.yaml` — existing Variant A skeleton
- `/Users/nick/openwhispr-server/compose/litellm/litellm_config.yaml` — Variant A default model list (OpenRouter + Groq + OpenAI)
- `/Users/nick/openwhispr-server/compose/litellm/litellm_config.realistic.yaml` — Variant C model overrides (Speaches)
- `/Users/nick/openwhispr-server/apps/api/src/routes/diarization.ts` — pyannote.ai async branch + Speaches sync branch toggle
- `/Users/nick/openwhispr-server/apps/api/src/routes/index.ts` (lines 380-411) — SPEACHES_DIARIZATION_URL env-driven registration
- `/Users/nick/openwhispr-server/apps/api/src/routes/transcribe.ts` (lines 1-60) — transcribe always via LiteLLM
- `/Users/nick/openwhispr-server/.env.example` (lines 98-116, 284-285) — HF_TOKEN comment block + outbound allowlists
- `/Users/nick/openwhispr-server/docs/operations.md` (lines 392, 456, 533) — only docs mention of HF_TOKEN/Speaches
