...
|---|---|---|
| `packages/byok-guard/src/index.ts` | `apps/api/src/lib/dep-check.ts` (header style) + `packages/email/` (package layout) | Boot-time guard SHARED by api + worker → workspace package, not `apps/api/src/lib/`. File-header style mirrors `dep-check.ts` (SPDX + Phase / Plan / Task tag + Source-of-truth pointer + Surface paragraph + behavior block). Package layout (`package.json` `main: ./src/index.ts`, `tsconfig.json` extends `../../tsconfig.base.json`, `vitest.config.ts`) mirrors `packages/email/` verbatim. Both `apps/api` and `apps/worker` import via `@openwhispr/byok-guard` (workspace:*). Cross-app relative imports (`../../api/src/lib/byok-guard.js`) are FORBIDDEN — they violate the `packages/email/` precedent and tsconfig project-reference boundaries. |
| `packages/byok-guard/src/__tests__/byok-guard.test.ts` | `apps/api/src/lib/dep-check.test.ts` | Vitest, no internal-logic mocks; only process-boundary mocks for `process.exit` via spy + in-memory pino destination for stderr capture. Pure env-string assertions — no PG / no network. |
