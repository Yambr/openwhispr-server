...
| `compose/docker-compose.ingress.yml` | same | Adds `traefik` + Traefik labels deltas on web/api + ACME volume + removes the host-port `4000:3000` / `3000:3000` published by slim-core (overlay last-wins on `ports:` would be problematic — instead overlay does NOT republish; host-port stays open behind Traefik for ops/health checks). Plan 14-03 finalizes. |
