...
- `/Users/dev/Downloads/server.py` — working Yandex Search Python reference. **CURRENTLY SANDBOXED by macOS — cannot be read directly by tooling.** Researcher: ask user to move into the repo (suggested `tools/reference/yandex-search-server.py`, gitignored if it contains keys) before Yandex adapter planning begins.
