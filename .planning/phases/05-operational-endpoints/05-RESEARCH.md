...
1. **Yandex Search API v2 wire shape (BLOCKING for Plan 04)**
   ...
   - **RESOLVED:** User must move `/Users/nick/Downloads/server.py` into the repo (e.g. `tools/reference/yandex-search-server.py`; gitignored if it contains live keys) before Plan 04 execution. Without it, the Yandex adapter ships at LOW confidence and may need a rewrite at integration time.
