...
1. **Yandex Search API integration** — public docs are CAPTCHA-blocked from automated tools; the user-provided Python reference at `/Users/nick/Downloads/server.py` cannot be read due to macOS Downloads-folder sandboxing (`EPERM`). Plan 03 (web-search) is BLOCKED on the user moving this file into the repo (e.g. `tools/reference/yandex-search-server.py`, gitignored if it contains live keys). Yandex Search API v1 is **deprecated since 2025-09-30**; v2 has both sync and deferred (async, operation-poll) modes — without the reference, we cannot confirm which mode this deployment uses. Tavily is **fully verified**.
...
// macOS sandboxes /Users/nick/Downloads/server.py with EPERM; planner MUST request
...
**Mitigation for A1-A3, A7 (Yandex):** Plan 04 BLOCKS on the user moving `/Users/nick/Downloads/server.py` → `tools/reference/yandex-search-server.py`. The planner MUST raise this as an OPEN-QUESTION-FOR-USER before generating Plan 04. Once the file is readable, all four assumptions resolve to VERIFIED or get corrected in a Plan 04 amendment.
...
1. **Yandex Search API v2 wire shape (BLOCKING for Plan 04)**
   ...
   - Recommendation: **User must move `/Users/nick/Downloads/server.py` into the repo (e.g. `tools/reference/yandex-search-server.py`; gitignored if it contains live keys) before Plan 04 execution.** Without it, the Yandex adapter ships at LOW confidence and may need a rewrite at integration time.
...
| User-provided Yandex reference (`/Users/nick/Downloads/server.py`) | Plan 04 Yandex adapter design | ✗ (macOS Downloads sandbox EPERM) | n/a | **BLOCKING — must move into repo** |
...
- `/Users/nick/openwhispr-server/.planning/phases/05-operational-endpoints/05-CONTEXT.md` — 36 locked design decisions (D-01..D-36).
- `/Users/nick/openwhispr/src/services/{Notes,Folders,Conversations,Transcriptions,ApiKeys}Service.ts` — authoritative wire shape via TypeScript interfaces (read in full).
- `/Users/nick/openwhispr/src/services/cloudApi.ts` — passthrough mechanics + V1Response envelope.
- `/Users/nick/openwhispr-server/apps/api/src/routes/transcribe.ts` — Phase 3 ledger idempotency pattern (lines 130-136); web-search and streaming-usage mirror this.
- `/Users/nick/openwhispr-server/apps/api/src/routes/realtime.ts` — Phase 4 env-key 503 + undici 3s/5s timeout pattern.
- `/Users/nick/openwhispr-server/packages/data/src/schema/usage_ledger.ts` — schema reused with new kinds (`streaming-stt`, `web-search.tavily`, `web-search.yandex`).
- `/Users/nick/openwhispr-server/packages/contract-tests/src/` — Phase 2-3-4 contract suite (24 existing test files); Phase 5 extends with 11 new files.
...
- These ALL resolve once `/Users/nick/Downloads/server.py` is moved into the repo by the user.
