...
- **C1:** ONE SAN cert covering all 5 hosts (`api.localhost`, `web.localhost`, `app.localhost`, `grafana.localhost`, `mailpit.localhost`). Matches existing `dynamic.yml` `/certs/local.crt` + `/certs/local.key` wiring byte-for-byte. ZERO YAML edits to existing Traefik configs.
