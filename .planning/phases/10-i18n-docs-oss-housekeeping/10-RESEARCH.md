...
- `/Users/dev/openwhispr-server/CLAUDE.md` — runtime i18n en+ru minimum from day one; source-artifact language English-only (hard rule, NON-NEGOTIABLE)
...
- `/Users/dev/openwhispr-server/.planning/REQUIREMENTS.md` — locked requirement IDs
- `/Users/dev/openwhispr-server/.planning/ROADMAP.md` §"### Phase 10" — 6 success criteria
- `/Users/dev/openwhispr-server/.planning/PROJECT.md` lines 211-230 — Key Decisions (16 rows for ADR-0004..0014)
- `/Users/dev/openwhispr-server/CLAUDE.md` — stack pin, constitutional rules
- `/Users/dev/openwhispr-server/apps/web/src/lib/i18n.ts` + `i18n-client.tsx` + `app/layout.tsx` — existing Phase-07.1 web scaffold (with the explicit "Phase 10" extension marker)
- `/Users/dev/openwhispr-server/apps/api/src/error-handler.ts` — `setErrorHandler` single chokepoint (Phase-2 D-13)
- `/Users/dev/openwhispr-server/apps/worker/src/jobs/email-delivery.ts` — payload schema already has `locale: z.enum(["en","ru"])`
- `/Users/dev/openwhispr-server/packages/data/src/schema/audit_log.ts` — 18-action enum, CHECK-constrained
- `/Users/dev/openwhispr-server/tools/lint-english.ts:59` — locale-files allowlist already in place
- `/Users/dev/openwhispr-server/docs/adrs/0000-template.md` — Nygard format pinned
