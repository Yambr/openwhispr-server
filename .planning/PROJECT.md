...
- **Upstream client repo**: `/Users/dev/openwhispr` — Electron desktop app, BYOK third-party AI, Better Auth client.
- **Authoritative wire spec**: `/Users/dev/openwhispr/docs/SELF_HOSTING.md` (walkthrough), `BACKEND_SPEC.md` (per-endpoint contract), `OAUTH_SPEC.md` (auth flows). 1556 lines total — this is the source of truth.
- **LiteLLM/Speaches reference**: `/Users/dev/openwhispr-server/speaches-audio.md` — production deployment at ExampleCorp on `llm.internal.example.com`. Three audio routes (Whisper transcription, pyannote diarization, Speaches Realtime WSS), one virtual key covers all. Includes a known LiteLLM v1.82.3 multipart-passthrough bug + backport patch.
...
| Default backend = LiteLLM + Speaches | Production-proven stack (ExampleCorp); covers Whisper transcription, pyannote diarization, OpenAI-Realtime-spec WSS in one image | — Pending |
