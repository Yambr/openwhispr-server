...
**Note:** `speaches-audio.md` references the bug but **does not say the fix is unmerged**; the PR was already in v1.83.7-stable when that doc was written. The patch file in the Alfaleasing repo is a relic of their v1.82.3 deployment timeline.
...
Run Speaches in its own container (matches Alfaleasing's prod setup per `speaches-audio.md`). Three roles in one image:
