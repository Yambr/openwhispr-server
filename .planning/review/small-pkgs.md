...
- **Status:** CLOSED 2026-05-21 — Phase 68, commit `4cda5f6c` — resolved DOC-ONLY (fix shape (a)). Verify-first grep of all 3 `EmailSender` callers confirmed none interpolates user-controlled data into `html`: the worker `email-delivery.ts` passes HTML rendered by `template-renderer` with `htmlEscape: true`; `apps/api/src/auth.ts` (lines 561 + 613) interpolates only a Better-Auth-generated reset/verify URL. A boundary escape would double-escape the worker's already-safe HTML, so no escape was added. The caller-owns-escaping contract is now explicit in the `SendArgs.html` JSDoc + a dedicated HTML-escaping section in `packages/email/README.md`.
