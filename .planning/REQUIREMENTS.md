...
| LOCKER-06 | 31 / 36.a | WARN-only-pending-Phase-36.a | `tools/lint-shell-credential-interpolation.ts` shipped WARN-only in Plan 31-06; BLOCKING flip lands with Phase 36.a (CRIT-FIX-05). |
...
- [ ] **LOCKER-06**: `tools/lint-shell-credential-interpolation.ts` — landed WARN-only; refuses template-literal strings passed to `spawn('bash', ['-c', ...])` / `execSync` / `exec` / `execFileSync` referencing `*_URL`, `*_KEY`, `*_PASSWORD`, `*_SECRET`, `*_TOKEN` bindings. Coverage ≥ 90/90/90/90. BLOCKING flip lands with Phase 36.a (CRIT-FIX-05). — Phase 31 / Plan 31-06.
