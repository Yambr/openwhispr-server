...
| Requirement | Phase | Status | Notes |
|-------------|-------|--------|-------|
| LOCKER-01 | 31 | Complete | `tools/lint-no-env-branches.ts` BLOCKING + tests at ≥ 90/90/90/90 — Phase 31 / Plan 31-01 |
| LOCKER-02 | 31 | Complete | `tools/lint-no-suppressions.ts` BLOCKING + tests + seeded allowlist (36 entries) — Phase 31 / Plan 31-02 |
| LOCKER-03 | 31 | Complete | `tools/lint-no-hardcode.ts` BLOCKING + tests + seeded allowlist (47 entries across 5 buckets) — Phase 31 / Plan 31-03 + 31-08 (retag 3 docker-compose-internal-url defaults) |
| LOCKER-04 | 31 / 41 | WARN-only-pending-Phase-41 | `tools/lint-prod-readiness.ts` shipped WARN-only in Plan 31-04; BLOCKING flip operationally deferred to Phase 41 closure (47-route bulkfix backlog + 469 dead-export → Phase 38). Rationale: `31-08-DECISIONS.md §D-1`. |
| LOCKER-05 | 31 / 37 | BLOCKING (flipped Phase 37, 2026-05-16) | `tools/lint-secret-shape-in-error.ts` — `--warn-only` dropped from `package.json`; 3 seed entries (LitellmUpstreamError + 2 pyannote-client siblings) removed from allowlist after fix landed. `pnpm lint:lockers` exits 0 on HEAD. |
| LOCKER-06 | 31 / 36.a | BLOCKING (flipped Phase 36.a, 2026-05-16) | `tools/lint-shell-credential-interpolation.ts` — `--warn-only` dropped; 3 audit-archive seed entries removed; 11 pre-existing test/tooling entries documented in `36-a-DECISIONS.md` §D-1. `pnpm lint:lockers` exits 0 on HEAD. Commit `92ece0d`. |
...
| CRIT-FIX-01 | 32 | Closed (2026-05-16) | Migration `0018_rls_fail_closed.sql` + **128-case** (16 tables × 4 ops × 2 ctx) property test on real Postgres testcontainer — Source: `data.md` CR-01 + HI-04 |
| CRIT-FIX-02 | 33 | Closed (2026-05-16) | Migration pair `0019_envelope_encrypt_secret_columns_add.sql` (additive) + Node backfill (Plan 33-03) + `0020_envelope_encrypt_secret_columns_drop_plaintext.sql` (final) + Drizzle lens + `lint-no-plaintext-secret-columns` (DISCIPLINE Rule 15 / LOCKER-PLAINTEXT-COLS / LOCKER-08) + `docs/security.md` §12 — Phase 33 / Plan 33-05 atomic closure. — Source: `data.md` CR-02 |
| LOCKER-PLAINTEXT-COLS | 33 | Complete | `tools/lint-no-plaintext-secret-columns.ts` (LOCKER-08) refuses `text` / `varchar` / `char` columns whose name matches the 8 credential set in `packages/data/src/schema/**` — Phase 33 / Plan 33-05 atomic commit; DISCIPLINE Rule 15 |
| CRIT-FIX-03 | 34 | Complete (2026-05-16) | `tenantPlugin` deleted (preferred path; audit confirmed zero production readers of `req.tenantId`); module-augmentation `FastifyRequest.tenantId` removed; e2e `tests/e2e/tenant-isolation.test.ts` asserts forged `x-tenant-id` cannot set `req.tenantId`; regression test in `apps/api/tests/unit/__tests__/no-tenant-plugin-regression.test.ts` — Source: `api-core.md` CR-01 |
| CRIT-FIX-04 | 35.a | Complete (2026-05-16) | `config: { auth: false }` on `/api/auth/providers` + `/api/setup-state` (locale already opted out in 19b/SR-19b.3); full-stack integration test boots `buildApp({auth, db})` and asserts 200 (not 401) anonymously. — Source: `api-routes-rest.md` CR-01. Commit `b9a4e6e`. |
| CRIT-FIX-05 | 35.b | Complete (2026-05-16) | `Headers.getSetCookie()` loop in `better-auth-handler.ts` replaces `forEach`-only path; non-Set-Cookie headers continue via guarded forEach. Two-cookie test asserts exactly 2 independent set-cookie response headers (not comma-joined). — Source: `api-routes-rest.md` CR-02. Commit `7b46659`. |
| CRIT-FIX-06 | 35.c | Complete (2026-05-16) | `setup-admin` step-4 role flip wrapped in compensating try/catch: DELETEs half-created user → UPDATEs setup_state back to pending → returns 503 `ADMIN_CREATE_FAILED` envelope. Real-Postgres testcontainer regression with Proxy-wrapped owner pool that throws on `UPDATE users SET role` only. — Source: `api-routes-rest.md` CR-03. Commit `79a6768`. |
| CRIT-FIX-07 | 36.a | Complete (2026-05-16) | `audit-archive.ts` rewritten — argv-array `spawn()` + PG* env vars + node:stream piping; password+dbUrl redacted from thrown errors; LOCKER-06 BLOCKING flipped in same commit. Coverage 98.6/91.1/94.1/100 on diff. — Source: `worker.md` CR-01. Commit `92ece0d`. |
| CRIT-FIX-08 | 36.b | Complete (2026-05-16) | `reconciliation-discrepancy.ts` honest windowed backfill — `runIngestOnce(deps, {since,until,tenantId})` extended; closure-captured result replaces double-cast; tenant-scoped + windowed SQL. Coverage 100/100/100/100. — Source: `worker.md` CR-02. Commit `d36818e`. |
| CRIT-FIX-09 | 37 | Complete (2026-05-16) | `LitellmUpstreamError.bodyText` truncated+private+non-enumerable+`toJSON()` override; +2 siblings in `pyannote-client.ts` fixed identically; LOCKER-05 flipped WARN→BLOCKING in same closing commit. Coverage errors.ts 100/100/100/100; pyannote-client.ts 98.78/98.03/100/98.78. — Source: `litellm-client.md` CR-01 |
| CRIT-FIX-10 | 38 | Complete (2026-05-16) | `packages/auth/` renamed `@openwhispr/auth` → `@openwhispr/auth-stub` (was already `private: true`). Load-bearing namespace unsquattable. `isPlaceholder()` export retained as Stryker mutation target. — Source: `small-pkgs.md` CR-01. Commit `c843b8a`. |
| HIGH-FIX-WIRE-01 | 39 | Complete (2026-05-16) | `.strict()` on every input zod schema (NoteInput, FolderInput, ConversationInput + nested messages, TranscriptionInput, StreamingUsageBody, WebSearchRequest, CreateApiKeyOptions). Property tests reject unknown keys. — Source: `wire-schemas.md` HI-1. Commit `a0ee7cb`. |
| HIGH-FIX-WIRE-02 | 39 | Complete (2026-05-16) | Output schemas use `.uuid()` (id, conversation_id, client_*_id), `.datetime({offset:true})` (created_at, updated_at, deleted_at, archived_at, expires_at, last_used_at), `.url()` (WebSearchResult.url). — Source: `wire-schemas.md` HI-2. Commit `a0ee7cb`. |
| HIGH-FIX-WIRE-03 | 39 | Complete (2026-05-16) | Long-text bounded (content/enhanced_content 256 KB, transcript 5 MB, prompt 16 KB, message content 256 KB, transcription text 5 MB); metadata record bounded to scalar values + 4 KB stringified cap. — Source: `wire-schemas.md` HI-3, HI-4. Commit `a0ee7cb`. |
| HIGH-FIX-WIRE-04 | 39 | Complete (2026-05-16) | Symmetrical enums (CloudNote.note_type = NoteTypeSchema; CloudTranscription.status = TranscriptionStatusSchema; SttConfig.availableProviders = SttProviderSchema; NoteRecordingConfig.allowedFormats = AudioFormatSchema). Counts/durations now `.int().nonnegative()` or `.nonnegative().finite()`. diarization_enabled tightened to `0|1`. — Source: `wire-schemas.md` HI-5, HI-6. Commit `a0ee7cb`. |
| HIGH-FIX-BYOK-01 | 40 | Complete | Wire schemas moved to `@openwhispr/wire-schemas`; `@openwhispr/contract-tests` `private: true` — Source: `byok-guard-contract-tests.md` HI-1 |
| HIGH-FIX-BYOK-02 | 40 | Complete | `redactUrl` query-string + SigV4 + bearer-in-path + drift-as-failure parity test — Source: `byok-guard-contract-tests.md` HI-2 |
| HIGH-FIX-BYOK-03 | 40 | Complete | `fetchAndParse` envelope enforcement on non-2xx — Source: `byok-guard-contract-tests.md` HI-3 |
...
- [x] **LOCKER-01**: `tools/lint-no-env-branches.ts` — BLOCKING; refuses any `process.env.NODE_ENV` / `NODE_ENV` comparison in `apps/**/src/**` and `packages/**/src/**` outside `bootstrap.ts`, `config/*.ts`, `otel-bootstrap.ts`, `*.config.ts`. Coverage ≥ 90/90/90/90. Wired into Lefthook + GitHub Actions. — Phase 31 / Plan 31-01.
- [x] **LOCKER-02**: `tools/lint-no-suppressions.ts` — BLOCKING; refuses `as any`, `as unknown as`, `@ts-ignore`, `@ts-nocheck`; requires `@ts-expect-error` to carry `issue-NNNN: <reason>` format. Allowlist seeded with 36 entries; CI fails on net additions via `tools/lockers-allowlist-diff.ts`. Coverage 98.63/96.15/100/98.5. — Phase 31 / Plan 31-02.
- [x] **LOCKER-03**: `tools/lint-no-hardcode.ts` — BLOCKING; refuses hardcoded `localhost`, `127.0.0.1`, `:3000`/`:4000`/`:8080`, UUID literals, fake-token shapes (`sk-…`, `sk-ant-…`, `AIza…`, `AKIA…`, `Bearer ey…`) outside `tests/`, `.env.*.example`, `compose/`, `docs/`, `charts/`, `tools/`. Allowlist seeded with 47 entries across 5 buckets. Coverage 97.18/93.93/100/100. — Phase 31 / Plan 31-03 + Plan 31-08 (retag 3 docker-compose-internal-url defaults).
- [ ] **LOCKER-04**: `tools/lint-prod-readiness.ts` — landed WARN-only; AST scan of `apps/**/src/**` + `packages/**/src/**`: (a) every Fastify `app.route/get/post/...` MUST have `schema: { body|querystring|params: <ZodSchema> }` AND `config: { rateLimit: ... }` (or `rateLimit: false` for `/api/health|/api/ready|/api/healthz|/livez|/readyz|/startupz|/api/_test/*`); (b) every exported symbol MUST have ≥ 1 non-test importer. Coverage 96.37/92.30/96.77/98.00. **BLOCKING flip operationally deferred from Plan 31-08 to Phase 41 closure** (47-route bulkfix backlog tagged `route-bulkfix-31-08` + 469 dead-export tagged `dead-export-phase-38`). Rationale: `31-08-DECISIONS.md §D-1`. Nightly `lockers-nightly` job invokes the BLOCKING form daily.
- [ ] **LOCKER-05**: `tools/lint-secret-shape-in-error.ts` — landed WARN-only; refuses `class X extends Error { public/readonly <bodyText|responseBody|upstreamPayload|response|body>: string }` unless constructor truncates the field. Coverage ≥ 90/90/90/90. BLOCKING flip lands with Phase 37 (CRIT-FIX-09). — Phase 31 / Plan 31-05.
- [x] **LOCKER-06**: `tools/lint-shell-credential-interpolation.ts` — BLOCKING (flipped Phase 36.a, 2026-05-16, commit `92ece0d`); refuses template-literal strings passed to `spawn('bash', ['-c', ...])` / `execSync` / `exec` / `execFileSync` referencing `*_URL`, `*_KEY`, `*_PASSWORD`, `*_SECRET`, `*_TOKEN` bindings. Coverage ≥ 90/90/90/90. — Phase 31 / Plan 31-06 + Phase 36.a flip.
...
- [x] **CRIT-FIX-01** (Phase 32, CLOSED 2026-05-16): Migration `0018_rls_fail_closed.sql` reverses 0003's `ALTER ROLE openwhispr_app SET app.tenant_id` + drops the GUC-bound `tenant_id` column DEFAULT on the four Better Auth tables. RLS policies use `tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid` (NULLIF avoids the PG-planner short-circuit hazard). Property test (**128 = 16 tables × 4 ops × 2 ctx**) on real Postgres testcontainer asserts silent-deny-read + raise-on-write (variant a). Route-level cleanup of legitimate empty-read paths deferred to Phase 41 per `.planning/phases/32-rls-fail-closed/32-DEFERRED.md`. — Source: `data.md` CR-01, HI-04.
- [x] **CRIT-FIX-02** (Phase 33, CLOSED 2026-05-16): Migration pair `0019_envelope_encrypt_secret_columns_add.sql` (additive — adds 48 nullable bytea sidecars matching the `EncryptedRow` shape `{dek_wrapped, dek_iv, dek_auth_tag, value_iv, value_auth_tag, value_ciphertext}` per credential × 8 credentials, plus `sessions.token_fp` + `previous_token_fp` SHA-256 fingerprint sidecars + partial-unique / partial indexes) + Node backfill (Plan 33-03) + `0020_envelope_encrypt_secret_columns_drop_plaintext.sql` (drops the 8 plaintext columns + `sessions_token_unique`, flips `sessions.token_fp` to NOT NULL, promotes `sessions_token_fp_unique` to full UNIQUE) converts `account.{access_token, refresh_token, id_token, password}`, `verification.value`, `sessions.{token, previous_token}`, `oauth_state.code_verifier` to envelope-encrypted `bytea` (AES-256-GCM, per-row DEK, `MASTER_KEK` env). Drizzle-level encryption lens wires `packages/data/src/encryption/envelope.ts` + `lens.ts` + `oauth-state-codec.ts`. Boot validator (`validateEncryptionBoot()`) gates api+worker entries with EX_CONFIG 78 on missing/invalid KEK. `tools/lint-no-plaintext-secret-columns.ts` (DISCIPLINE Rule 15 / LOCKER-PLAINTEXT-COLS / LOCKER-08) prevents future text-column drift. Operator docs in `docs/security.md` §12 (scope, MASTER_KEK setup, KEK rotation runbook, KMS provisioning for AWS/GCP/Azure/Vault, rollback rescue). — Source: `data.md` CR-02.
- [x] **LOCKER-PLAINTEXT-COLS** (Phase 33, CLOSED 2026-05-16): `tools/lint-no-plaintext-secret-columns.ts` + DISCIPLINE Rule 15 + `CLAUDE.md` Engineering Discipline mirror + lefthook + Makefile + ci.yml + nightly.yml wiring. BLOCKING from day one; no `--warn-only`, no allowlist. Phase 33 / Plan 33-05 atomic closure commit.
- [x] **CRIT-FIX-03** (Phase 34, CLOSED 2026-05-16): `apps/api/src/middleware/tenant.ts` + its `index.ts` registration deleted via the preferred path. Audit (`.planning/phases/34-tenant-plugin-retirement/34-AUDIT.md`) confirmed zero production readers of `req.tenantId` — Phase 2 dual-auth migrated every authenticated route to the authoritative `req.tenant` (populated by `dualAuthHook` from the resolved Better Auth session). E2E `tests/e2e/tenant-isolation.test.ts` (3 assertions) proves forged / missing / array-valued `x-tenant-id` cannot decorate `req.tenantId`. Regression test `apps/api/tests/unit/__tests__/no-tenant-plugin-regression.test.ts` (3 assertions: file absence, no top-level `tenantId` augmentation, no executable `req.tenantId` reads in production source) guards against re-introduction. — Source: `api-core.md` CR-01.
- [x] **CRIT-FIX-04** (Phase 35.a): `apps/api/src/routes/{auth-providers,setup-state}.ts` add `config: { auth: false }` (locale already opted out in 19b/SR-19b.3). Integration test `apps/api/tests/unit/integration/public-bootstrap-endpoints.test.ts` boots `buildApp({auth, db})` with a fake auth resolving no session and asserts each of the three routes returns 200 anonymously. — Source: `api-routes-rest.md` CR-01. Commit `b9a4e6e` (2026-05-16).
- [x] **CRIT-FIX-05** (Phase 35.b): `apps/api/src/routes/better-auth-handler.ts` replaces `Headers.forEach`-only path with `Headers.getSetCookie()` per-value emission + guarded forEach for non-Set-Cookie headers. Test asserts 2 independent `set-cookie` reply headers (no comma-joined "<name>=<value>, <name>=" signature) and zero-cookie case emits no set-cookie header. — Source: `api-routes-rest.md` CR-02. Commit `7b46659` (2026-05-16).
- [x] **CRIT-FIX-06** (Phase 35.c): `apps/api/src/routes/setup-admin.ts` step-4 role flip wrapped in compensating try/catch — DELETEs half-created user → UPDATEs `setup_state` back to `pending` → returns 503 `ADMIN_CREATE_FAILED` envelope. Real-Postgres testcontainer regression in `apps/api/tests/unit/routes/__tests__/setup-admin-rollback.test.ts` with Proxy-wrapped owner pool that throws on `UPDATE users SET role` only. Existing 10 setup-admin tests continue to pass. — Source: `api-routes-rest.md` CR-03. Commit `79a6768` (2026-05-16).
- [x] **CRIT-FIX-07** (Phase 36.a, CLOSED 2026-05-16): `apps/worker/src/jobs/audit-archive.ts` rewritten — argv-array `spawn()` + PG* env vars + node:stream piping; password+dbUrl scrubbed from thrown error `.message`/`.stack` via `redactSecret()`. LOCKER-06 BLOCKING flipped in same atomic commit. 20/20 audit-archive tests GREEN; coverage 98.6/91.1/94.1/100 on diff. — Source: `worker.md` CR-01. Commit `92ece0d`.
- [x] **CRIT-FIX-08** (Phase 36.b, CLOSED 2026-05-16): `apps/worker/src/jobs/reconciliation-discrepancy.ts` rewritten — `runIngestOnce(deps, {since,until,tenantId})` signature extended with explicit-window SQL path filtering on `startTime >= since AND startTime < until` and tenant-scoped via owner-pool subquery; closure-captured result replaces the previous double-cast; watermark not advanced in windowed mode. 8/8 reconciliation tests GREEN + 15/15 ingest tests GREEN (regression preserved); coverage 100/100/100/100 on reconciliation-discrepancy. — Source: `worker.md` CR-02. Commit `d36818e`.
- [x] **CRIT-FIX-09** (Phase 37, CLOSED 2026-05-16): `packages/litellm-client/src/errors.ts` truncates `bodyText` at construction (`.slice(0, 200)`) + marks `private readonly` + sets `enumerable: false` via `Object.defineProperty` + adds `toJSON()` override returning `{name, message, status}` only. +2 siblings (`PyannoteBadRequestError`, `PyannoteUpstreamError` in `apps/api/src/lib/pyannote-client.ts:68,80`) fixed identically. Tests assert `JSON.stringify(new <Class>(500, 'x'.repeat(10000)))` < 500 bytes, no 201-char body run, and `err.toJSON()` shape. LOCKER-05 flipped WARN→BLOCKING in the closing commit (allowlist cleared, `--warn-only` dropped from `package.json`). Coverage errors.ts 100/100/100/100; pyannote-client.ts 98.78/98.03/100/98.78. — Source: `litellm-client.md` CR-01.
- [x] **CRIT-FIX-10** (Phase 38, CLOSED 2026-05-16): `packages/auth/` renamed `@openwhispr/auth` → `@openwhispr/auth-stub` (was already `private: true`); load-bearing namespace is unsquattable. `isPlaceholder()` export retained as Stryker mutation target; `stryker.config.json` glob unchanged. `vitest.config.ts:63` project name aligned. Zero non-self importers of `@openwhispr/auth` before rename (audit clean). — Source: `small-pkgs.md` CR-01. Commit `c843b8a`.

### High-severity sweep (Phases 39–41)

- [x] **HIGH-FIX-WIRE-01** (Phase 39, CLOSED 2026-05-16): `.strict()` on every input zod schema in `packages/wire-schemas/` (NoteInput, FolderInput, ConversationInput + nested messages, TranscriptionInput, StreamingUsageBody, WebSearchRequest, CreateApiKeyOptions). Property tests reject unknown keys. Commit `a0ee7cb`.
- [x] **HIGH-FIX-WIRE-02** (Phase 39, CLOSED 2026-05-16): Output schemas use `z.string().uuid()` for ids + client_*_ids + conversation_id, `.datetime({offset:true})` for *_at fields, `.url()` for WebSearchResult.url. Property tests reject malformed strings. Commit `a0ee7cb`.
- [x] **HIGH-FIX-WIRE-03** (Phase 39, CLOSED 2026-05-16): Long-text body fields bounded (content 256 KB, transcript/text 5 MB, prompt 16 KB); metadata records bounded to scalar values + 4 KB stringified cap. Commit `a0ee7cb`.
- [x] **HIGH-FIX-WIRE-04** (Phase 39, CLOSED 2026-05-16): Symmetrical enums (CloudNote.note_type, CloudTranscription.status, SttConfig.availableProviders, NoteRecordingConfig.allowedFormats); counts/durations `.int().nonnegative()` or `.nonnegative().finite()`; diarization_enabled `0|1`. Commit `a0ee7cb`.
- [x] **HIGH-FIX-BYOK-01** (Phase 40): Wire schemas moved from `@openwhispr/contract-tests` into `@openwhispr/wire-schemas`; `contract-tests` becomes `private: true`. API routes import from `wire-schemas` only.
- [x] **HIGH-FIX-BYOK-02** (Phase 40): `redactUrl` covers query-string credentials (`api_key`, `token`, `key`, `code`, `secret`, AWS SigV4 `X-Amz-Signature`), URL userinfo, and bearer-token-shaped path segments (`sk-…`, `sk-ant-…`, `AIza…`, `AKIA…`). Drift-as-failure parity test against every `process.env.*_API_KEY` actually read by the codebase.
- [x] **HIGH-FIX-BYOK-03** (Phase 40): `fetchAndParse` envelope enforcement removes the `typeof body === "object"` guard; non-JSON / empty body raises `MalformedUpstreamEnvelopeError`.
