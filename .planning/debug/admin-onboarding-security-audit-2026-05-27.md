...
`grep -n -E "email_verified|emailVerified|verify" /Users/nick/openwhispr-server/apps/api/src/routes/setup-admin.ts` returns ZERO matches. The handler:
...
`grep -n -E "recordAudit|admin\.role_changed" /Users/nick/openwhispr-server/apps/api/src/routes/setup-admin.ts` returns ZERO matches. The 18-action audit enum (`packages/data/src/schema/audit_log.ts`) DOES include `admin.role_changed` with payload `{ target_user_id, before, after }` (audit.ts:163-167). The setup-admin handler does NOT call `recordAudit('admin.role_changed', ...)` after promoting the first user to admin.
