...
- [ ] **Phase 09.1: Real kind Apply (Live Chart Validation)** — IN PROGRESS 2026-05-13. Phase 9 closed with 106/106 helm-unittest GREEN but `helm install` against a live cluster was never executed on the maintainer's machine. This phase boots a real `kind v0.31` cluster + CNPG + cert-manager + Traefik, runs `helm dependency update` + `helm install --wait` + `helm test`, captures every wiring bug as a RED helm-unittest assertion → fix template → GREEN inline TDD cycle, and proves DEPLOY-05 first-launch SLO probe <5 min wall-time. See `.planning/phases/09.1-helm-real-kind-apply/09.1-PLAN.md`.
