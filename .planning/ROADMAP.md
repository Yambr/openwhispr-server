...
- [x] **Phase 15: Repo Refactor + FSL Relicense + History Scrub** — CLOSED-WITH-DEFERRED-SCRUB 2026-05-25 (`status: human_needed-accepted`, 12/14 codebase deliverables MET; 2 FSL-06+FSL-07 force-push artefacts deferred). All non-scrub work landed: Test-layout codified (`tests/{unit,integration}/`), `compose/` directory consolidation, Traefik host split (`web.localhost`/`api.localhost` closes TD-15.g `/api/locale` 404 shadowing), Apache-2.0 → FSL-1.1-ALv2 via `reuse` codemod across ~675 SPDX headers + workspace `package.json` + Docker LABELs + README badges + DCO sign-off + retroactive contributor consent + ADR-0013. `tools/history-scrub.sh` + `docs/runbooks/15-04-history-scrub.md` runbook driver ready. **Force-push DEFERRED 2026-05-25** per user direction (`Defer Phase 15 — human_needed-accepted`): repo public since 2026-05-08 (`github.com/Yambr/openwhispr-server` 0 forks/0 stars/0 network), `speaches-audio.md` is operator-doc not credentials/PII, history-scrub script has pre-flight bug (`set -e` + subshell pipe falsely reports "not found") needing fix before re-attempting. Scrub remains operator-runnable when desired; not gating OSS publish. Working-tree delete already staged. See `.planning/phases/15-repo-refactor-fsl-relicense-history-scrub-v2/15-VERIFICATION.md`.
- [ ] **Phase 53: Browser-console scraping in every e2e + fix web→api routing 404 cascade** — OPENED 2026-05-18 after manual smoke surfaced two bugs invisible to the existing e2e suite: (a) `POST /api/auth/sign-up/email` from web origin returns 404 (Next.js routes own `/api/*`, no proxy to backend api on :4000), (b) inline-script CSP `'unsafe-inline'` violation on sign-up page. Phase ships `tests/e2e-cjm/support/browser-diagnostics.ts` capturing 4 dimensions (console messages, page errors, network 4xx/5xx, CSP violations) attached in EVERY browser-driving e2e via Before/beforeEach hook; failed e2e surfaces the captured browser-error before the assertion message; `allowBrowserErrors([pattern])` escape hatch for negative-twins. Plans: 53-01 RED sentinel (4 dimensions) → 53-02 GREEN helper → 53-03 sweep existing e2e → 53-04 DISCUSS web→api routing (3 candidates: Next.js rewrites / Traefik host-split default / absolute NEXT_PUBLIC_API_URL) → 53-05/06 RED→GREEN @cjm-web-signup-1 → 53-07 CSP nonce/external-file fix. See `.planning/phases/53-web-console-scrape-and-api-routing/53-PLAN.md`.
- [x] **Phase 16: Phase-Tag Comment Audit** — CLOSED 2026-05-15 (`status: passed`, 4/4 must-haves verified). `tools/phase-tag-sweep.ts` codemod (5 REMOVE + 5 KEEP rules + conservative-KEEP default) + `tools/lint-phase-tag-comments.ts` regression linter wired into `pnpm`/lefthook/CI. Empirical sweep found 23 violations (vs ROADMAP-cited ~754 — most absorbed by Phase 15 SPDX rewrite churn); collapsed to single atomic commit `001e9ac7 refactor(16-02): sweep 23 phase-tag comments per CLAUDE.md`. 4 review findings (1 CRITICAL + 3 WARNING) all FIXED inline. VERIFICATION at `.planning/phases/16-phase-tag-comment-audit-v2/16-VERIFICATION.md`. SUMMARY authored retroactively 2026-05-25 to close disk-tracking gap.
...
### v2.3 — Cross-repo client contract conformance (milestone opened 2026-05-19)

Source: `/Users/nick/openwhispr/.planning/phases/08-client-server-audit/SERVER-REQUIREMENTS.md` (12 requirements R1..R12) produced by upstream Electron client's Phase 8 + Phase 9 closures. One-directional rule: server adapts, client doesn't migrate (client repo is upstream-parity and refuses bridging patches). Pre-OSS, zero deprecation cost.
...
- [x] **Phase 57: Audit security + CI-blocker fixes** — CLOSED 2026-05-20 (`status: closed`, 9/9 Tier-0 CRITICALs resolved + `data:CR-02` D2 documented; 2 deferred to Phase 58 as Tier-1). 14 atomic commits across 6 fix tracks (A-F): A.1+A.2 envelope encryption lens wraps Better Auth transaction adapter; B `data:CR-02` D2 RLS posture documented as v1 single-tenant accepted debt; C `/api/_test/*` production veto; D byok redact extensions; E `INGRESS_BASE_URL`/`AUTH_URL` boot-required; F production safety knobs refuse-to-boot under `NODE_ENV=production`. Closing commit `bf5fabde`. SUMMARY at `.planning/phases/57-pre-publication-critical-fixes/57-SUMMARY.md` (authored retroactively 2026-05-25 to close disk-tracking gap).
- [x] **Phase 58: Audit hardening + library adoption** — CLOSED 2026-05-20 (`status: closed`, 4/4 Tier-1 CRITICALs resolved; `data:CR-04` residual dual-auth → Phase 59). 4 RED→GREEN TDD pairs: A `worker:CR-01` watermark hold on recoverable skips; B `worker:CR-02` rollup + reconciliation by `event_at` (migration `0027`); C `data:CR-04` regression lock; D `data:CR-05` dead oauth-state-codec plaintext fallback removal. Plus AUDIT-HARD-01..05 + AUDIT-LIB-01..03 + AUDIT-DOC-01 mechanical sweep. Full-suite triage: zero net new test failures vs main. Closing commit `99eae7c6`. SUMMARY at `.planning/phases/58-remaining-critical-fixes/58-SUMMARY.md` (authored retroactively 2026-05-25).
- [x] **Phase 59: Documentation truth pass** — CLOSED 2026-05-21 (`status: passed`). 15 doc-vs-code lies fixed across README quickstart, `litellm-target-spec`, `architecture.md`, `CONTRIBUTING.md`, `security.md`, `observability.md`, `wire-contracts-phase-3.md`; 5 stale TECH_DEBT.md entries pruned. Also resolved `data:CR-04` residual dual-auth Better Auth bearer session.token wiring (commit `29528220 fix(R20+R19)`) + `R13` seed-tenant dual-auth carve-out (`b4636dc0`). SUMMARY at `.planning/phases/59-client-e2e-server-followups/59-01-SUMMARY.md`.
- [x] **Phase 60: CI builds + smoke tests for OSS publish** — CLOSED 2026-05-22 (`status: passed`). GitHub Actions release build workflow (multi-arch api/web/worker images), smoke-test job booting embedded-litellm compose stack, cross-reference wiring to upstream `openwhispr` Electron client repo. SUMMARY at `.planning/phases/60-api-logging-and-oauth-callback-fix/60-01-SUMMARY.md`.
- [x] **Phase 61: Load tests for publish** — CLOSED 2026-05-22 (`status: closed-with-deferred-reconciliation`). `make load-smoke` (mock-profile k6) run surfaced systemic rot in load-test path — 6 distinct pre-existing bugs accumulated since Phase 8 (Phase 14 slim-core split + BYOK guard + env-template restructuring drift). **4 FIXED** atomic (commits `804b6dcb` observability overlay layering + `aa27f965` POSTGRES_ADMIN_URL sslmode + 2 storage/ingress overlay fixes); **2 DEFERRED** per CLAUDE.md Hard Rule 2 (`.env.full.example` INGRESS_TLS_CERT_PATH gap + `.env.slim.example` POSTGRES_OWNER_USER/APP_USER gap — both require env-template/compose-default contract reconciliation phase with own TDD cycle). Published SLO numbers (`docs/operations.md` Phase-8 Run-5) remain valid — hardware-bound, OPERATOR_RERUN_ON_GPU. Status doc: `.planning/audit-v2.4/PHASE-61-LOADTEST-STATUS.md`. Closing commit: `765428da docs(61): mark Phase 61 closed in STATE.md`.
- [x] **Phase 62: OSS publish** — CLOSED 2026-05-25 (de-facto effective via `git push origin main` at repo creation 2026-05-08; verification reconciled 2026-05-25). Repo public at `github.com/Yambr/openwhispr-server` since 2026-05-08 (`createdAt: 2026-05-08T17:30:10Z`); origin/main synchronized with local HEAD (84b90245); chart releases publishing via `chart-releaser` workflow on `chart-v*` / `openwhispr-server-*` tags (1.0.0..1.0.7 + postgres 1.0.0..1.1.0 on GHCR). 0 forks / 0 stars / 0 network as of close. Phase 15 FSL history-scrub deferred (operator one-shot when desired; not gating publish — `speaches-audio.md` is operator-doc, no credentials). README polish + UI screenshots tracked as Phase 62.x optional adornment (deferred-items DEF-62-README-POLISH), not a blocker. NOTE: phase number 62 also exists in v2.5 HIGH-findings sweep namespace (`62-high-findings-api-core`) — distinct phases sharing numeric label; v2.4 Phase 62 detail section at `### Phase 62: OSS publish (v2.4)` line ~1388.
...
Plans:
- [x] 69-01-PLAN.md — Wave 0: pure JIT config loader (oidc-jit-config.ts) + config/ JSON-parse boot validator + pure resolveJitDecision (7 rejection codes, tie-break, revocation), RED-first, 100% branch [SSO-IMPL-02, SSO-IMPL-01]
- [x] 69-02-PLAN.md — Wave 1: audit enum 18->21 + 3 no-PII zod payloads + migration 0032 (DROP/ADD CHECK without ONLY) + down, one atomic TDD commit (D-69-2) [SSO-IMPL-04]
- [x] 69-03-PLAN.md — Wave 2: auth.ts mapProfileToUser + 4 databaseHooks + tenantId additionalField; 5 rejection codes->403/400 + 3 sso.jit.* audit events; real PG/PgBouncer/Valkey testcontainers (D-69-1 web, D-69-2) [SSO-IMPL-03, SSO-IMPL-04]
- [x] 69-04-PLAN.md — Wave 3: desktop mint-bearer seam — widen scope+userinfo, call shared resolveJitDecision, pass tenantId/role into createOAuthUser; SSRF/token/rotation/channel-scheme unchanged (D-69-1 desktop) [SSO-IMPL-01, SSO-IMPL-03]
- [x] 69-05-PLAN.md — Wave 1: realm-openwhispr-test.json (separate path, userinfo group mapper) + Admin-REST seed script (LOCKER-06) + keycloak.yml wired into make e2e-cjm; split @cjm-sso-1.5 into 1.5a/1.5b (D-69-3) [SSO-IMPL-05]
- [ ] 69-06-PLAN.md — Wave 4: real sso.steps.ts (7 scenarios) + sibling unit tests + un-red feature + update sso-step-drift.test.ts; live-Keycloak @sso e2e 7/7 GREEN incl. desktop bearer deep-link [SSO-IMPL-05] — **CODE+WIRING COMPLETE & committed (c994a496/71259734/8a4078fe), unit+drift 14/14 GREEN; HALTED at the live run on a quay.io keycloak:26.0 image-pull / network-egress timeout (environment block). Re-run `make e2e-cjm SCENARIO=@sso` once registry egress is available.**
