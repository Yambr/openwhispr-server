...
- [ ] **Phase 08.6: Speaches main-branch build + local diarization wiring** — Speaches `latest-cpu` Docker tag (v0.8.2) does NOT expose `/v1/audio/diarization` (verified empirically via /openapi.json). The endpoint exists on `main` branch. Switch compose to `build: from main` (or pin a SHA), then wire api `apps/api/src/routes/diarization.ts` to bypass pyannote.ai (paid) and point at Speaches local `/v1/audio/diarization` (with pyannote/speaker-diarization-community-1 model) via a new SPEACHES_DIARIZATION_URL env. LiteLLM Enterprise pass_through_endpoints not required — api calls Speaches direct. INSERTED 2026-05-13.
- [ ] **Phase 08-08: docs/operations.md + SLO publication** — finalize Plan 08-08: publish per-endpoint p95 SLO budgets (Mac mock baseline from Run 5 commit a5e5920 with OPERATOR_RERUN_ON_GPU markers), document sizing matrix, document load-test profiles (mock for plateaus, realistic for smoke proof-of-wiring), close Phase 8 umbrella. UNBLOCKED 2026-05-13 after 08.4 + 08.5 closures.
