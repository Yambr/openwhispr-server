...
- [ ] **Phase 08.5: realistic profile boot + smoke + short Mac baseline** — per user directive 2026-05-13: realistic profile (Speaches + Whisper-large-v3 + pyannote per `speaches-audio.md` / `docs/litellm-target-spec.md`) must boot end-to-end locally, run k6 smoke (5 VU × 60s per endpoint) and a short Mac baseline (5+5+2 min) so the wiring is proven and operator can re-run on H100 by swapping numbers, not wiring. All tests preserved for operator re-run. INSERTED 2026-05-13. **PARTIAL 2026-05-13**: Waves 1+2 CLOSED (8 atomic commits land wiring); Wave 3 BLOCKED on operator `.env` provider keys — see `08.5-03-STATUS.md` for unblock recipe.
