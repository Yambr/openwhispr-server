...
**Success Criteria** (what must be TRUE):
  1. Migration `0018_envelope_encrypt_secret_columns.sql` exists; adds `*_ciphertext` / `*_dek_wrapped` / `*_iv` / `*_tag` bytea columns; backfills from plaintext via tested Node migrator; drops plaintext columns in a follow-up commit AFTER the encryption lens is wired and integration tests green. Forward + rollback both verified on real Postgres in CI.
