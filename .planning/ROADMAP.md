...
- [ ] **Phase 33: Envelope encryption wired to Better Auth credential columns (CR-8 closure)** — Migration pair `0019_envelope_encrypt_secret_columns_add.sql` (additive — adds 48 nullable bytea sidecars + `sessions.token_fp` / `previous_token_fp` SHA-256 fingerprints + partial-unique indexes) + Node-side backfill (Plan 33-03) + `0020_envelope_encrypt_secret_columns_drop_plaintext.sql` (drops the 8 plaintext columns + `sessions_token_unique` after the lens is wired and integration tests green) converts `account.{access_token, refresh_token, id_token, password}`, `verification.value`, `sessions.{token, previous_token}`, `oauth_state.code_verifier` to envelope-encrypted `bytea` (AES-256-GCM, per-row DEK, 12-byte IV, GCM tag, KEK from `MASTER_KEK` env). The currently-dead `packages/data/src/encryption/envelope.ts` is wired as a Drizzle-level encryption lens. New locker `tools/lint-no-plaintext-secret-columns.ts` (becomes DISCIPLINE Rule 15) prevents future text-column drift. Operator docs in `docs/security.md` cover encryption scope + `MASTER_KEK` rotation + KMS provisioning (AWS/GCP/Azure/Vault). Source: CRIT-FIX-02 (`.planning/review/data.md` CR-02). **Depends on Phase 32** — RLS posture must be fail-closed before encrypted bytea columns get the same RLS multiplier.
- [ ] **Phase 34: tenantPlugin retirement (CR-1 closure)** — `apps/api/src/middleware/tenant.ts` + its registration at `apps/api/src/index.ts:382` deleted (preferred) OR renamed to `req.untrustedTenantHint: string | null` with a runtime guard throwing on `req.tenant ≠ req.untrustedTenantHint`. Client-controlled `x-tenant-id` header can no longer escalate access. E2E `tests/e2e/tenant-isolation.spec.ts` asserts a forged header is REFUSED. Module-augmentation `FastifyRequest.tenantId` audited and removed. Source: CRIT-FIX-03 (`.planning/review/api-core.md` CR-01).
- [ ] **Phase 35: api-routes-rest bundle (CR-2 + CR-3 + CR-4 closure)** — Three sub-plans: **35.a** add `config: { auth: false }` to `apps/api/src/routes/{locale,auth-providers,setup-state}.ts` so the public bootstrap endpoints stop 401-ing under the global `dualAuthHook` (CRIT-FIX-04 / `api-routes-rest.md` CR-01); **35.b** replace `Headers.forEach((v,k) => reply.header(k,v))` in `apps/api/src/lib/better-auth-handler.ts:179-182` with `headers.getSetCookie()` per-value emission so multi-cookie Better Auth responses don't get comma-joined into one broken cookie (CRIT-FIX-05 / `api-routes-rest.md` CR-02); **35.c** wrap `apps/api/src/routes/setup-admin.ts:234` step-4 role flip + `setup_state=completed` in a single transaction with rollback so a `pg` failure does not wedge the instance into `alreadyCompleted=true` with no admin user (CRIT-FIX-06 / `api-routes-rest.md` CR-03).
- [ ] **Phase 36: worker bundle (CR-5 + CR-6 closure)** — Two sub-plans: **36.a** `apps/worker/src/jobs/audit-archive.ts:96-128` replaces `spawn('bash', ['-c', script])` with Node-side `spawn('pg_dump', [...args])` + `PGPASSWORD` env + piped streams to `gzip`/`mc`/`aws` — eliminates DATABASE_URL-with-password from `ps aux`, BullMQ `failedReason`, and Loki structured logs (CRIT-FIX-07 / `worker.md` CR-01); **36.b** `apps/worker/src/jobs/reconciliation-discrepancy.ts:45-61` either implements the windowed backfill properly (read `since/until/tenant_id` from payload; extend `runIngestOnce` signature; remove the `as unknown as` cast; return real `{rowsProcessed, rowsScanned}`) OR deletes the job + BullMQ enqueuer with documented rationale in SUMMARY.md (CRIT-FIX-08 / `worker.md` CR-02).
- [ ] **Phase 37: LitellmUpstreamError bodyText truncation (CR-9 closure)** — `packages/litellm-client/src/errors.ts:31, 40` truncates `bodyText` at construction (`.slice(0, 200)`), makes the field `private readonly`, and overrides `toJSON()` to return `{name, message, status}` only — so pino's own-property serializer cannot exfiltrate full upstream bodies to Loki. Regression test asserts `JSON.stringify(new LitellmUpstreamError(500, 'x'.repeat(10000)))` < 500 bytes; pino structured-log contains no `bodyText` field. Source: CRIT-FIX-09 (`.planning/review/litellm-client.md` CR-01). Locker LOCKER-05 (`lint-secret-shape-in-error`) catches future regressions of the same class.
- [ ] **Phase 38: @openwhispr/auth retirement (CR-10 closure)** — `packages/auth/` deleted (preferred — only its own tests import it) OR renamed to `@openwhispr/auth-stub` with `private: true` in `package.json` so it cannot be published under a load-bearing name. Stryker config audited for stale references. Source: CRIT-FIX-10 (`.planning/review/small-pkgs.md` CR-01). Locker LOCKER-04 part (b) (dead-export detection) catches future similar shells.
- [ ] **Phase 39: wire-schemas HIGH sweep (HIGH-FIX-WIRE-01..04 closure)** — Mechanical sweep across `packages/wire-schemas/`: (a) `.strict()` on every input schema (NoteInput, FolderInput, ConversationInput, TranscriptionInput, StreamingUsageBody, WebSearchRequest, CreateApiKeyOptions); (b) `z.string().uuid()` / `.datetime({offset:true})` / `.url()` on permissive output primitives; (c) `.max()` bounds on long-text body fields + `metadata` records per BACKEND_SPEC limits; (d) symmetrical enums on `note_type` (and siblings); `z.number().int().nonneg()` for counts/durations. Property tests reject unknown keys + malformed strings; existing contract suite must remain green. Source: HIGH-FIX-WIRE-01..04 (`.planning/review/wire-schemas.md` HI-1..6).
- [ ] **Phase 40: byok-guard + contract-tests HIGH sweep (HIGH-FIX-BYOK-01..03 closure)** — (a) Package-boundary inversion: every wire schema currently imported from `@openwhispr/contract-tests` by `apps/api/src/routes/**` moves into `@openwhispr/wire-schemas`; `contract-tests` flipped to `private: true` (HIGH-FIX-BYOK-01); (b) `redactUrl` extended to mask query-string credentials (`api_key`, `token`, `key`, `code`, `secret`), AWS SigV4 `X-Amz-Signature`, URL userinfo, and bearer-token-shaped path segments (`sk-…`, `sk-ant-…`, `AIza…`, `AKIA…`); drift-as-failure parity test enumerates every `process.env.*_API_KEY` actually read by `apps/**/src/**` (HIGH-FIX-BYOK-02); (c) `fetchAndParse` envelope enforcement — the `typeof body === "object"` guard removed; non-JSON / empty body raises `MalformedUpstreamEnvelopeError` (HIGH-FIX-BYOK-03). Source: `.planning/review/byok-guard-contract-tests.md` HI-1..3.
- [ ] **Phase 41: Residual HIGH sweep (HIGH-FIX-API-CORE / AGENT-STREAM / WEB / WORKER / DATA / LITELLM / SMALL closure)** — Seven sub-plans, each RED → GREEN → REFACTOR atomic commit: **41.a** api-core — replace hardcoded `"00000000-..."` at `apps/api/src/auth.ts:330, 380` with `resolveDefaultTenantId()`; delete `apps/api/src/placeholder.ts` (HIGH-FIX-API-CORE / `api-core.md` HI-01..03); **41.b** `/api/agent/stream` — reconcile `DEFAULT_AGENT_MODEL` with LiteLLM config (single source of truth), add body zod validation, add per-user `rateLimit` (HIGH-FIX-AGENT-STREAM / `api-routes-transcriptions.md` HI-01..03); **41.c** web — app-level RSC role-check guard on `/admin/*` (defense-in-depth over Traefik basic-auth); remove `PLAYWRIGHT_DISABLE_SSR_PREFETCH` from 5 production RSC pages (HIGH-FIX-WEB / `web.md` HI-1..2); **41.d** worker — bare `pino()` replaced with shared redact factory in `index.ts` + `ingest-litellm-spend.ts`; reconciliation-daily-check loop bound corrected; OTel gauge callbacks read fresh `driftStore`; minutes-priced model `metadata.duration` validation + warn-log + counter metric (HIGH-FIX-WORKER / `worker.md` HI-1..4); **41.e** data — `migrate.ts` LiteLLM-init idempotency enforced; migration `0019` replaces 0005's TRUNCATE with idempotent UPSERT; account-token `expires_at` enforcement (HIGH-FIX-DATA / `data.md` HI-01..03; HI-04 closed by Phase 32); **41.f** litellm-client — `headersTimeout`/`bodyTimeout`/required `AbortSignal` on `chatCompletions`, `audioTranscriptions`, `passthrough`; SSRF dispatcher asserted at module load; model-alias drift fixed via single-source-of-truth read from `compose/litellm/litellm_config.yaml`; `streamOptions` spread allows caller opt-out of `include_usage` (HIGH-FIX-LITELLM / `litellm-client.md` HI-01..04); **41.g** small-pkgs — real en/ru locale bundles OR `packages/i18n` renamed to `-stub` (verify against Phase 10 coverage); CI parity test between `byok-guard` and `observability/redact` provider lists; `SMTP_SECURE` parser accepts `1`/`true`/`yes`/`on` case-insensitive (HIGH-FIX-SMALL / `small-pkgs.md` HI-01..03).
...
- [ ] **Phase 21: Anti-shortcut Locker Infrastructure (CRITICAL — must land before any other Phase 22+; complementary to Phase 31 Constitutional Lockers)** — 5 new linters wired into pre-commit + CI + branch-protection so future agents cannot ship `.skip`/`.only` Gherkin, `retries > 0`, step files without unit tests, `[test-fix]` PRs that edit production code, or per-phase coverage below 90/90/90/90. New scripts: `tools/lint-gherkin-tags.ts`, `tools/lint-playwright-config.ts`, `tools/lint-steps-have-unit-tests.ts`, `tools/lint-no-prod-edit-with-test-only-pr.ts`, `tools/lint-coverage-floor-per-phase.ts`. Each ships RED→GREEN per constitutional TDD with sibling `*.test.ts` at 90/90/90/90. Wired into `package.json` scripts, `lefthook.yml` pre-commit, `.github/workflows/ci.yml` as required GHA jobs, `scripts/branch-protection.json` (adds 5 required checks → 21 total), `.github/CODEOWNERS` pins on sensitive paths, `.github/PULL_REQUEST_TEMPLATE.md` QA checklist section.
- [ ] **Phase 22: L1 Smoke layer (HIGH)** — `make smoke` synthetic-transaction probe between `docker compose up --wait` and `make e2e-cjm`. New `tests/smoke/` with 5 probes (health, transcribe 415 typed envelope, realtime WSS auth-gate close 4401, web root, traefik host-split). `vitest.smoke.config.ts`. New CI job `smoke` between `up --wait` and `e2e-cjm`; on failure dumps `docker compose logs --tail=200` per memory `feedback_check_loki_after_tests`. Target wall-clock < 5 s total.
- [ ] **Phase 23: L2 BYOK provider matrix (HIGH)** — `tests/integration/byok-provider-matrix.test.ts` covers 4 providers (OpenAI, OpenRouter, Groq, Bedrock) × 2 BYOK overlay states = 8 permutations. Testcontainers-driven LiteLLM with mock upstream (no paid calls — per memory `feedback_loadtest_cost_discipline`, paid gated behind `OPENWHISPR_LOADTEST_ALLOW_PAID`). Asserts upstream provider routing per env var. `packages/litellm-client/` coverage stays 90/90/90/100.
- [ ] **Phase 24: G8 cross-tenant RLS CJM (non-SSO)** — `tests/e2e-cjm/features/rls-cross-tenant.feature` + steps + `__tests__/`. `@cjm-15.1` + `@cjm-15.2` anchored in `docs/customer-journeys.md`. Reuses `support/fixtures.ts` `freshTenant()`. Expected GREEN today (no new prod code). No phase dependency — runs in parallel with 25..29.
- [ ] **Phase 25: G5 agent-stream NDJSON CJM** — `tests/e2e-cjm/features/agent-stream.feature` + `@cjm-12.1`/`@cjm-12.2` anchors. Reuses `compose/mock-litellm/` SSE fixture from Phase 08.2. Coverage closure only — Phase 4 already ships NDJSON.
- [ ] **Phase 26: G6 web-search CJM (Tavily/Yandex via mock)** — `tests/e2e-cjm/features/web-search.feature` + `@cjm-13.1`/`@cjm-13.2`. Mock provider only (memory `feedback_loadtest_cost_discipline`). Coverage closure — Phase 5 already ships routes.
- [ ] **Phase 27: G7 session refresh / set-auth-token CJM** — `tests/e2e-cjm/features/session-refresh.feature` + `@cjm-14.1`/`@cjm-14.2`. `@sinonjs/fake-timers` for token-rotation threshold crossing. Asserts `set-auth-token` header per `BACKEND_SPEC.md §4`.
- [ ] **Phase 28: G3 diarization round-trip CJM** — `tests/e2e-cjm/features/diarization.feature` + `@cjm-10.1`/`@cjm-10.2`. `tests/fixtures/diarize-2speakers.wav` (5–10 s synthetic). Tag `@after-docker-up @after-speaches-main` — Phase 08.6 already ships the Speaches-main build per memory `feedback_speaches_diarization_build_from_main`.
- [ ] **Phase 29: G4 realtime WSS user-journey CJM** — `tests/e2e-cjm/features/realtime-stream.feature` + `@cjm-11.1`/`@cjm-11.2`. Reuses :8443 entrypoint from Phase 08.4. `ws` client library already in repo via `tools/load-test/`.
- [ ] **Phase 30: G1 LiteLLM virtual-key rotation CJM (depends on Phase 23)** — `tests/e2e-cjm/features/byok-key-rotation.feature` + `@cjm-byok-rotation.1`/`@cjm-byok-rotation.2`. Tagged `@expected-red @after-phase-LITELLM-04` if route absent; else GREEN. Per `CLAUDE.md` Hard Rule §1 do NOT add the route here — coverage phase only.
- [ ] **Phase 42: G2 per-tenant STT/LLM override CJM (depends on Phase 23 + 19.2)** — `tests/e2e-cjm/features/tenant-settings-override.feature` + `@cjm-9.1`/`@cjm-9.2`. Cross-refs WIRE-11/12/28 (Phase 5 implemented).
- [ ] **Phase 43: G9 transcribe via corporate `LITELLM_BASE_URL` override CJM (depends on Phase 19.2)** — `tests/e2e-cjm/features/byok-corporate-litellm.feature` + `@cjm-byok-litellm.1`. New `compose/mock-corp-litellm/` second mock at different port; reuses `compose-harness.ts` `envOverrides`.
- [ ] **Phase 44: L3 PR-time k6 mock load smoke (≤ 2 min)** — `Makefile` target `load-smoke`; CI job runs `PROFILE=mock DURATION=60s VUS=5` on every PR. Baselines committed in `tests/load/baselines/`. Cost-discipline asserted: no `OPENWHISPR_LOADTEST_ALLOW_PAID` reads.
- [ ] **Phase 45: L4 consolidate vitest configs** — merge `vitest.config.ts` + `vitest.e2e.config.ts` into single config with conditional `E2E` env gate. Per-package strict 90/90/90/90 thresholds preserved.
- [ ] **Phase 46: L5 testcontainers cleanup self-test** — `tests/self-tests/testcontainers-cleanup.test.ts` validates no orphan testcontainers remain after `pnpm test` exits. Catches Ryuk-not-firing regressions per memory `feedback_testcontainers_cleanup_audit`.
- [ ] **Phase 47: L6 SSO step-string drift self-test** — `tests/self-tests/sso-step-drift.test.ts` diffs step strings between `sso.steps.ts` placeholder stubs and `keycloak-oidc.feature` Gherkin. Catches drift before Phase 18/v3 SSO implementation lands.
- [ ] **Phase 48: L7 SR-19a.4 worker S3 normative fix** — promote `compose/docker-compose.storage.yml` to inject `S3_ENDPOINT` on worker (not only api). Remove non-normative override block from `tests/e2e-cjm/compose-overrides.yml`. Verify `@cjm-byok-storage.*` scenarios still GREEN.
- [ ] **Phase 49: L8 weekly @expected-red staleness alert** — `.github/workflows/expected-red-staleness.yml` cron job parses every `@expected-red @after-phase-X` tag, cross-references ROADMAP planned dates, opens/updates a tracking issue if any scenario stayed RED past its phase ETA.
- [ ] **Phase 50: G10 billing/subscription CJM scaffold (deferred to v3)** — anchor reservation for `@cjm-billing-*` namespace in `docs/customer-journeys.md` + `tests/e2e-cjm/features/_deferred-billing.md` placeholder + `.planning/deferred-items.md` entry. No `.feature` file. Closure = doc-only commit.
