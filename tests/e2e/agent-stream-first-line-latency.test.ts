...
describe("e2e — POST /api/agent/stream first-line latency (WIRE-07 SC#1)", () => {
  it(
    "first NDJSON byte arrives < 500ms ROUND-TRIP through real Traefik chain",
    async () => {
      ...
      // eslint-disable-next-line no-console
      console.log(
        `[WIRE-07 SC#1] lines=${lines.length} max-per-line-gap=${maxDelta.toFixed(2)}ms`,
      );
    ...
    // Per-test override: 60s is plenty (compose stack is already up;
    // this test only exercises one POST). Keeps regression noise low
