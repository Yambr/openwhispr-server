...
Speaches master задеплоен на `llm.internal.example.com`
...
Все три используют один upstream-ключ — `EXAMPLECORP_API_KEY`
(общий для всех `aimodels` моделей). LiteLLM подменяет `Authorization`
при форварде, клиенту это не видно.

Hosts:
- **Prod**: `https://llm-api.internal.example.com`
- **Dev**: `https://llm-api.dev.internal.example.com`
...
|---|---|---|
| `examplecorp/whisper-large-v3-russian` | ru (default) | `/v1/audio/transcriptions` |
| `examplecorp/whisper-large-v3-english` | en (alias к той же модели) | `/v1/audio/transcriptions` |
| `examplecorp/canary-1b-v2` | multi-lang (Russian by default) | `/v1/canary/audio/transcriptions` |

### Пример

```bash
curl -X POST https://llm-api.internal.example.com/v1/audio/transcriptions \
  -H "Authorization: Bearer sk-YOUR-KEY" \
  -F file=@meeting.wav \
  -F model=examplecorp/whisper-large-v3-russian
```
...
```bash
curl -X POST https://llm-api.internal.example.com/v1/audio/diarization \
  -H "Authorization: Bearer sk-YOUR-KEY" \
  -F file=@meeting.wav \
...
`WSS /v1/realtime?model=examplecorp/speaches-realtime` — потоковая
...
async def main():
    url = "wss://llm-api.internal.example.com/v1/realtime?model=examplecorp/speaches-realtime"
...
прописывайте `examplecorp/speaches-realtime`, для diarization
...
```bash
curl -X POST https://llm-api.internal.example.com/key/generate \
