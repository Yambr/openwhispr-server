...
import { canRunDocker } from "../../../../../apps/worker/src/lib/can-run-docker.js";
...
// The suite boots real Postgres + Valkey testcontainers — it requires a
// reachable docker daemon. `canRunDocker()` probes DOCKER_HOST + the
// well-known socket paths (same gate the worker testcontainer suites
// use). When docker is absent (a laptop without Docker Desktop) the
// suite is skipped at file granularity rather than failing CI.
const READY = migrationsReady();
const DOCKER_OK = canRunDocker();
const SUITE = READY && DOCKER_OK ? describe : describe.skip;
...
async function ensureSharedBullMQ(): Promise<void> {
  ...
  const handler = withTenantContext(JOB_SCHEMA, guarded, async (data) => {
    ...
    try {
      ...
      // Resolve the per-run promise ONLY after COMMIT lands. Resolving
      // before COMMIT was the documented flake source — the test-side
      // `SELECT count(*) FROM notes` (expecting 2) could race ahead of
      // the second job's still-uncommitted INSERT and observe 1.
      if (currentRun) {
...
SUITE("worker-tier RLS property (D-W4 layer 3, fast-check)", () => {
  // Phase 57 — re-enabled. The historical flake ("expected 2,
  // received 1") was a test-harness race: the BullMQ handler resolved
  // its per-run promise BEFORE issuing `COMMIT`, so the verification
  // `SELECT count(*) FROM notes` could observe the second job's
  // still-uncommitted INSERT. Fixed by moving `resolveA/resolveB` to
  // after `COMMIT` (see `ensureSharedBullMQ` handler). The suite is now
  // gated on docker availability via `describe.skipIf` semantics
  // (`SUITE = READY && DOCKER_OK ? describe : describe.skip`).
  test.prop(
    {
