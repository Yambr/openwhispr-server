...
SUITE("Phase 5 / Plan 01 — migration 0006 backfill", () => {
  ...
  it("default tenant from 0000_initial.sql has a tenant_settings row after 0006", async () => {
  ...
  it("backfill is idempotent (no duplicate rows for the default tenant)", async () => {
    const pool = new Pool({ connectionString: ownerUri });
    try {
