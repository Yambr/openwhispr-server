...
const ListWithMessagesResponse = z.object({
  conversations: z.array(CloudConversationShape.extend({ messages: z.array(CloudMessageShape) })),
});
...
describe.skipIf(!REACHABLE)("WIRE-24 + WIRE-25 — /api/conversations/* (6 routes)", () => {
  ...
  it("GET /api/conversations/list?include=messages embeds CloudMessage[] per conversation (D-27)", async () => {
    ...
    const res = await jar.fetch(`${BACKEND_URL}/api/conversations/list?include=messages&limit=20`);
    expect(res.status).toBe(200);
