...
import { ErrorEnvelope, UsageResponse } from "../../src/schemas.js";
...
describe.skipIf(!REACHABLE)("WIRE-10 — GET /api/usage", () => {
  ...
  it("reflects ledger writes — POST /api/streaming-usage then GET /api/usage shows incremented sum", async () => {
    const jar = await signInFixture("fixture@conformance.test");
    const before = UsageResponse.parse(await (await jar.fetch(`${BACKEND_URL}/api/usage`)).json());
    ...
    const after = UsageResponse.parse(await (await jar.fetch(`${BACKEND_URL}/api/usage`)).json());
