...
describe("TranscribeResponse", () => {
  ...
  it("rejects limitReached: true (literal false in v1 per WIRE-05)", () => {
    expect(
      TranscribeResponse.safeParse({ ...valid, limitReached: true }).success,
    ).toBe(false);
  });
...
describe("ReasonRequest", () => {
  ...
  it("rejects unknown fields (.strict)", () => {
    expect(
      ReasonRequest.safeParse({ text: "hi", surprise: 1 }).success,
    ).toBe(false);
  });
});
...
describe("ReasonResponse", () => {
  ...
  it("response is forward-compat (extra fields allowed — no .strict)", () => {
    expect(
      ReasonResponse.safeParse({ ...valid, requestId: "abc" }).success,
    ).toBe(true);
  });
});
